...
function git(args) {
  return cp.execFileSync("git", args, { cwd: ROOT, encoding: "utf8", stdio: ["ignore", "pipe", "inherit"] }).trim();
}
...
async function main() {
  ...
  while (Date.now() < deadline) {
    const response = await fetch(url, { headers: { Accept: "application/vnd.github+json", "User-Agent": "airdev-packages-split-waiter" } });
    ...
    if (!run || run.status !== "completed") {
      if (run) console.log(`  ${run.status}: ${run.html_url}`);
      await sleep(5000);
      continue;
    }
    if (run.conclusion !== "success") throw new Error(`Split workflow ${run.conclusion}: ${run.html_url}`);
